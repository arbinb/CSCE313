...
void* worker_function(NRC* worker, BoundedBuffer* buffer, HistogramCollection* hist) {
    /*
...
int main(int argc, char* argv[]) {
    ...
    if (GlobalStruct->fileReq) {
        ...
        *(filemsg*)package = *file_msg;
        ...
        NRC* chanTmp = new NRC(GlobalStruct->hostName, GlobalStruct->port);
        chanTmp->cwrite((char*)&package, size);
        int length = *(int*)chanTmp->cread();
        int fd = open(output.c_str(), O_RDWR | O_CREAT, 0777);
        close(fd);
        patients.push_back(thread(fileProducer, length, request_buffer));

        MESSAGE_TYPE q = QUIT_MSG;
        chanTmp->cwrite((char*)&q, sizeof(MESSAGE_TYPE));
    } else {
        //data
    ...
    for (int i = 0; i < GlobalStruct->workers; i++) {
        ...
        NRC* worker = new NRC(GlobalStruct->hostName, GlobalStruct->port);
